...
int main (int argc, char *argv[])
{
   ...
   if (myid == 0)
   {
      cout << "\nenter order (0 for isoparametric space) --> " << flush;
      cin >> p;
   }
   MPI_Bcast(&p, 1, MPI_INT, 0, MPI_COMM_WORLD);
   FiniteElementCollection *fec;
   if (p)
      fec = new H1_FECollection(p, pmesh->Dimension());
   else
      if (pmesh->GetNodes())
   ...
   int glob_ndofs = fespace->GlobalTrueVSize();
   if (myid == 0)
      cout << "\nGlobal number of dofs : " << glob_ndofs
           << "\n\nAssembling ... " << flush;
   ...
   a->Assemble();
   {
      ...
      cout << "\nproc. " << myid << ": small elements: "
           << mat.CountSmallElems(1e-9*mat.MaxNorm())
           << '/' << mat.NumNonZeroElems() << endl;
   }
   if (myid == 0)
      cout << "b.c. ... " << flush;
   {
