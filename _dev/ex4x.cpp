...
int main (int argc, char *argv[])
{
   ...
   do
   {
      cout << "Enter k (for RTk) = " << flush;
      cin >> k;
   }
   while (k < 0 || k > 32);
   fec = new RT_FECollection(k, dim);
   // fec = new RT1_2DFECollection;
   ...
   ess_bdr = 1;
   x = 0.0;
   x.ProjectBdrCoefficientNormal(F, ess_bdr);
   ...
   GSSmoother M(A);
   // x = 0.0;
   PCG(A, M, *b, x, 1, 50000, 1e-24, 0.0);

   // when using proj. as initial guess:
   // PCG(A, M, *b, x, 1, 10000, 1e-12, 0.0);

   // CG(A, *b, x, 1, 50000, 1e-24, 0.0);
