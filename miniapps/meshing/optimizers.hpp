...
protected:
   ...
   int lbm;
public:
   LBFGSOptimizer(const IntegrationRule &irule,
...
void LBFGSOptimizer::SetOperator(const Operator &op)
{
   oper = &op;
   height = op.Height();
   width = op.Width();
...
void LBFGSOptimizer::Mult(const Vector &b, Vector &x) const
{
...
double LBFGSOptimizer::ComputeScalingFactor(const Vector &x,
