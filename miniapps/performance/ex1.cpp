...
const int            sol_p    = 1;              // solution order (default: 3)
...
typedef TBilinearForm<mesh_t,sol_fes_t,int_rule_t,integ_t,true> auto4_HPCBilinearForm;
typedef TBilinearForm<mesh_t,sol_fes_t,int_rule_t,integ_t,false> auto1_HPCBilinearForm;
...
int main(int argc, char *argv[])
{
   ...
   auto4_HPCBilinearForm *a_hpc_simd = NULL;
   auto1_HPCBilinearForm *a_hpc_scalar = NULL;
   ...
   else
   {
      ...
      if (simd)
        a_hpc_simd = new auto4_HPCBilinearForm(integ_t(coeff_t(1.0)), *fespace);
      else
        a_hpc_scalar = new auto1_HPCBilinearForm(integ_t(coeff_t(1.0)), *fespace);
      if (matrix_free)
      {
        if (simd)
