//                                MFEM Example HdivSQUARE
//
// Compile with: make exHdivSQUARE
//
// Sample runs:  exHdivSQUARE ../data/square1.mesh
//               exHdivSQUARE ../data/square2.mesh
//
...
{
protected:
   const SparseMatrix *X0pc;
   const SparseMatrix *Xhatpc;
   const SparseMatrix *Sinvpc;

public:
   HybridDPGPreconditioner(const SparseMatrix & A1,
                           const SparseMatrix & A2,
                           const SparseMatrix & A3)
   {
      size=((A1.Width())+(A2.Width())+(A3.Width()));
...
int main (int argc, char *argv[])
{
   ...
   cin >> n;
   if (0 < n) {
      cout << "Warning, Hdiv norm Preconditioner currently does not support higher order numerical trace spaces. " << endl << flush ;
   }
   int t = n+dim;
...
//   bEPS=(F->GetData());
   ...
   Shat->Assemble();
   Shat->Finalize();
   const SparseMatrix &SMShat = Shat->SpMat();
   ...
   S0->Assemble();
   S0->Finalize();
   const SparseMatrix &SMS0 = S0->SpMat();
   ...
   Sinv->Finalize();
   const SparseMatrix &SMSinv = Sinv->SpMat();

   HybridDPGPreconditioner *B = new HybridDPGPreconditioner(SMS0,SMShat,SMSinv);

   MINRES(*A,*B, b,bigX,1,10000, 1e-12, 1e-24);
   GridFunction x;
   x.Update(x0space, bigX, 0);

   cout << "\n|| u_h(SP Exact Hdiv PC) - u ||_{L^2} = " << x.ComputeL2Error(e_sol)
        << '\n' << endl;
