//                                MFEM Example SchurSQUARE - Parallel Version
//
// Compile with: make exSchurSQUAREp
//
// Sample runs:  exSchurSQUAREp ../data/square1.mesh
//               exSchurSQUAREp ../data/square2.mesh
//
...
//               the primal unknowns, and AMS for Bhat^T Sinv Bhat.
//
...
//   Recipe for Preconditioner for Numerical Trace Space
      HypreParMatrix *hBhat = new HypreParMatrix(s2, s1, row1, col1, &Xhatpc);
      HypreParMatrix *hSinv = new HypreParMatrix(s2, part2, Sinvpc);
      hXhatpc=RAP(hSinv,hBhat);
      delete hBhat;
      delete hSinv;
...
//   bEPS=(F->GetData());
   ...
   HybridDPGPreconditioner *B = new HybridDPGPreconditioner(SMS0,SMBhat,SMSinv,dim,xhatspace,mesh);

   MINRES(*A,*B, b,bigX,1,10000, 1e-12, 1e-24);
   GridFunction x;
   x.Update(x0space, bigX, 0);

   cout << "\n|| u_h(SP AMG AMS Schur PC) - u ||_{L^2} = " << x.ComputeL2Error(e_sol)
        << '\n' << endl;
