                               ...
                               bool assembled)
{
   ...
   if (wrap)
   {
      MFEM_VERIFY(assembled, "Cannot wrap in PETSc's unassembled format");
...
{
   Init();
   BlockDiagonalConstructor(comm,row_starts,row_starts,diag,assembled,&A);
   // update base class
   height = GetNumRows();
   width  = GetNumCols();
}
...
{
   Init();
   BlockDiagonalConstructor(comm,row_starts,col_starts,diag,assembled,&A);
   // update base class
   height = GetNumRows();
   width  = GetNumCols();
}
