...
#ifdef HYPRE_BIGINT
   mfem_hypre_TFree(hypre_CSRMatrixI(csr_a));
   mfem_hypre_TFree(hypre_CSRMatrixJ(csr_a));
#endif
...
void HypreParMatrix::Destroy()
{
   ...
   if (diagOwner >= 0)
   {
      if (diagOwner & 1)
      {
         mfem_hypre_TFree(hypre_CSRMatrixI(A->diag));
         mfem_hypre_TFree(hypre_CSRMatrixJ(A->diag));
      }
      hypre_CSRMatrixI(A->diag) = NULL;
      hypre_CSRMatrixJ(A->diag) = NULL;
      if (diagOwner & 2)
      {
         mfem_hypre_TFree(hypre_CSRMatrixData(A->diag));
      }
   ...
   if (offdOwner >= 0)
   {
      if (offdOwner & 1)
      {
         mfem_hypre_TFree(hypre_CSRMatrixI(A->offd));
         mfem_hypre_TFree(hypre_CSRMatrixJ(A->offd));
      }
      hypre_CSRMatrixI(A->offd) = NULL;
      hypre_CSRMatrixJ(A->offd) = NULL;
      if (offdOwner & 2)
      {
         mfem_hypre_TFree(hypre_CSRMatrixData(A->offd));
      }
   ...
   if (colMapOwner >= 0)
   {
      if (colMapOwner & 1)
      {
          mfem_hypre_TFree(hypre_ParCSRMatrixColMapOffd(A));
      }
