...
double Mesh::GetElementVolume(int i) {
  ...
  const IntegrationRule &ir =
      IntRules.Get(GetElementBaseGeometry(i), et->OrderJ());
  double volume = 0.0;
...
void Mesh::GetCharacteristics(double &h_min, double &h_max, double &kappa_min,
                              double &kappa_max, Vector *Vh, Vector *Vk) {
  int i, dim, sdim;
  DenseMatrix J;
  double h, kappa;
  ...
  if (Vh) {
    Vh->SetSize(NumOfElements);
  }
  if (Vk) {
    Vk->SetSize(NumOfElements);
  }
...
// static method
void Mesh::PrintElementsByGeometry(int dim, const Array<int> &num_elems_by_geom,
                                   std::ostream &out) {
  for (int g = Geometry::DimStart[dim], first = 1;
       g < Geometry::DimStart[dim + 1]; g++) {
    if (!num_elems_by_geom[g]) {
      continue;
    }
    if (!first) {
      out << " + ";
...
void Mesh::GetFaceTransformation(int FaceNo, IsoparametricTransformation *FTr) {
  ...
  } else  // curved mesh
  {
    ...
    } else  // L2 Nodes (e.g., periodic mesh), go through the volume of Elem1
    {
      ...
      GetLocalFaceTransformation(face_type, GetElementType(face_info.Elem1No),
                                 FaceElemTr.Loc1.Transf, face_info.Elem1Inf);
      // NOTE: FaceElemTr.Loc1 is overwritten here -- used as a temporary

      face_el = Nodes->FESpace()->GetTraceElement(face_info.Elem1No, face_geom);
...
void Mesh::GetLocalPtToSegTransformation(IsoparametricTransformation &Transf,
                                         int i) {
...
void Mesh::GetLocalSegToTriTransformation(IsoparametricTransformation &Transf,
                                          int i) {
  const int *tv, *so;
...
void Mesh::GetLocalSegToQuadTransformation(IsoparametricTransformation &Transf,
                                           int i) {
  const int *qv, *so;
...
void Mesh::GetLocalTriToTetTransformation(IsoparametricTransformation &Transf,
                                          int i) {
  DenseMatrix &locpm = Transf.GetPointMat();
...
void Mesh::GetLocalTriToWdgTransformation(IsoparametricTransformation &Transf,
                                          int i) {
  DenseMatrix &locpm = Transf.GetPointMat();

  Transf.SetFE(&TriangleFE);
  //  (i/64) is the local face no. in the pri
  MFEM_VERIFY(i < 128, "Local face index "
                           << i / 64
...
void Mesh::GetLocalQuadToHexTransformation(IsoparametricTransformation &Transf,
                                           int i) {
  DenseMatrix &locpm = Transf.GetPointMat();
...
void Mesh::GetLocalQuadToWdgTransformation(IsoparametricTransformation &Transf,
                                           int i) {
  DenseMatrix &locpm = Transf.GetPointMat();
...
void Mesh::InitTables() {
  el_to_edge = el_to_face = el_to_el = bel_to_edge = face_edge = edge_vertex =
      NULL;
}
...
void Mesh::InitMesh(int _Dim, int _spaceDim, int NVert, int NElem,
                    int NBdrElem) {
  SetEmpty();
...
void Mesh::AddHexAsTets(const int *vi, int attr) {
  static const int hex_to_tet[6][4] = {{0, 1, 2, 6}, {0, 5, 1, 6},
                                       {0, 4, 5, 6}, {0, 2, 3, 6},
                                       {0, 3, 7, 6}, {0, 7, 4, 6}};
  int ti[4];
...
void Mesh::AddHexAsWedges(const int *vi, int attr) {
  static const int hex_to_wdg[2][6] = {{0, 1, 2, 4, 5, 6}, {0, 2, 3, 4, 6, 7}};
  int ti[6];
...
void Mesh::FinalizeCheck() {
  MFEM_VERIFY(vertices.Size() == NumOfVertices || vertices.Size() == 0,
              "incorrect number of vertices: preallocated: "
                  << vertices.Size() << ", actually added: " << NumOfVertices);
  MFEM_VERIFY(elements.Size() == NumOfElements,
              "incorrect number of elements: preallocated: "
                  << elements.Size() << ", actually added: " << NumOfElements);
  ...
  if (*old_elem_vert == NULL) {
    ...
    if (need_elem_vert) {
      *old_elem_vert = new Table;
      ...
      for (int i = 0; i < GetNE(); i++) {
        (*old_elem_vert)
            ->AddConnections(i, elements[i]->GetVertices(),
...
void Mesh::DoNodeReorder(DSTable *old_v_to_v, Table *old_elem_vert) {
  ...
  if (NumOfEdges) {
    fes->GetEdgeInteriorDofs(0, old_dofs);
  }
...
void Mesh::Finalize(bool refine, bool fix_orientation) {
  ...
  const bool may_change_topology = (refine && (Dim > 1 && (meshgen & 1))) ||
  ...
  if (Dim >= 2 && Dim == spaceDim) {
    ...
    for (int i = 0; i < num_faces; i++) {
      MFEM_VERIFY(faces_info[i].Elem2No < 0 || faces_info[i].Elem2Inf % 2 != 0,
...
// Deep copy the NCMesh.
  ...
  if (mesh.Nodes && copy_nodes) {
    ...
    FiniteElementSpace *fes_copy = new FiniteElementSpace(*fes, this, fec_copy);
...
Mesh::Mesh(double *_vertices, int num_vertices, int *element_indices,
           Geometry::Type element_type, int *element_attributes,
           int num_elements, int *boundary_indices,
           Geometry::Type boundary_type, int *boundary_attributes,
           int num_boundary_elements, int dimension, int space_dimension) {
  ...
  int boundary_index_stride =
      num_boundary_elements > 0 ? Geometry::NumVerts[boundary_type] : 0;
...
void Mesh::Loader(std::istream &input, int generate_edges,
  ...
  {
    ReadGmshMesh(input);
  } else if ((mesh_type.size() > 2 && mesh_type[0] == 'C' &&
              mesh_type[1] == 'D' && mesh_type[2] == 'F') ||
             (mesh_type.size() > 3 && mesh_type[1] == 'H' &&
              mesh_type[2] == 'D' && mesh_type[3] == 'F')) {
...
Mesh::Mesh(Mesh *mesh_array[], int num_pieces) {
  ...
  if (mesh_array[0]->NURBSext) {
    ...
    for (i = 0; i < num_pieces; i++) {
      ...
      // copy the vertices
      for (j = 0; j < m->GetNV(); j++) {
        vertices[lvert_vert[j]].SetCoords(m->SpaceDimension(), m->GetVertex(j));
      }
    }
...
Mesh::Mesh(Mesh *orig_mesh, int ref_factor, int ref_type) {
  ...
  MFEM_VERIFY(ref_type == BasisType::ClosedUniform ||
                  ref_type == BasisType::GaussLobatto,
...
void Mesh::SetCurvature(int order, bool discont, int space_dim, int ordering) {
  ...
  FiniteElementSpace *nfes =
      new FiniteElementSpace(this, nfec, space_dim, ordering);
        ...
        if (fix_it) {
          switch (GetElementType(i)) {
            ...
            case Element::QUADRILATERAL:
              ...
              break;
            default:
              MFEM_ABORT("Invalid 2D element type \"" << GetElementType(i)
                                                      << "\"");
              break;
          }
          fo++;
  ...
  if (Dim == 3) {
    ...
    for (i = 0; i < NumOfElements; i++) {
      ...
      switch (GetElementType(i)) {
        ...
        default:
          MFEM_ABORT("Invalid 3D element type \"" << GetElementType(i) << "\"");
          break;
      }
    }
  }
  ...
  if (Dim == 3) {
    for (int i = 0; i < NumOfBdrElements; i++) {
      ...
      switch (bdr_type) {
        ...
        default:
          MFEM_ABORT("Invalid 2D boundary element type \"" << bdr_type << "\"");
...
void Mesh::GetElementFaces(int i, Array<int> &fcs, Array<int> &cor) const {
  int n, j;
...
int Mesh::GetBdrElementEdgeIndex(int i) const {
  switch (Dim) {
    ...
    case 3:
      return be_to_face[i];
    default:
      mfem_error("Mesh::GetBdrElementEdgeIndex: invalid dimension!");
  }
  return -1;
}
...
void Mesh::GetBdrElementAdjacentElement(int bdr_el, int &el, int &info) const {
  ...
  switch (GetBdrElementBaseGeometry(bdr_el)) {
    ...
    case Geometry::SEGMENT:
      ...
      break;
    case Geometry::TRIANGLE:
      ori = GetTriOrientation(fv, bv);
      break;
    case Geometry::SQUARE:
      ori = GetQuadOrientation(fv, bv);
      break;
    default:
      MFEM_ABORT("boundary element type not implemented");
      ori = 0;
  }
  el = fi.Elem1No;
...
void Mesh::AddTriangleFaceElement(int lf, int gf, int el, int v0, int v1,
                                  int v2) {
  if (faces[gf] == NULL)  // this will be elem1
  {
...
void Mesh::AddQuadFaceElement(int lf, int gf, int el, int v0, int v1, int v2,
                              int v3) {
      ...
      switch (GetElementType(i)) {
        case Element::TETRAHEDRON: {
          for (int j = 0; j < 4; j++) {
            ...
            AddTriangleFaceElement(j, ef[j], i, v[fv[0]], v[fv[1]], v[fv[2]]);
          }
          break;
        }
        case Element::WEDGE: {
          ...
          for (int j = 2; j < 5; j++) {
            const int *fv = pri_t::FaceVert[j];
            AddQuadFaceElement(j, ef[j], i, v[fv[0]], v[fv[1]], v[fv[2]],
                               v[fv[3]]);
          }
          break;
        }
...
void Mesh::GenerateNCFaceInfo() {
  ...
  for (unsigned i = 0; i < list.slaves.size(); i++) {
    ...
    if (slave.index >= nfaces || slave.master >= nfaces) {
      continue;
    }
  ...
  for (i = 0; i < NumOfElements; i++) {
    ...
    switch (GetElementType(i)) {
      case Element::TETRAHEDRON: {
        for (int j = 0; j < 4; j++) {
          const int *fv = tet_t::FaceVert[j];
          el_to_face->Push(i, faces_tbl->Push(v[fv[0]], v[fv[1]], v[fv[2]]));
        }
        break;
      }
      case Element::WEDGE: {
...
#ifndef MFEM_USE_METIS_5
      METIS_PartGraphRecursive(&n, (idxtype *)I, (idxtype *)J, (idxtype *)NULL,
                               (idxtype *)NULL, &wgtflag, &numflag, &nparts,
                               options, &edgecut, (idxtype *)partitioning);
#else
      err = METIS_PartGraphRecursive(
          &n, &ncon, I, J, (idx_t *)NULL, (idx_t *)NULL, (idx_t *)NULL, &nparts,
          (real_t *)NULL, (real_t *)NULL, options, &edgecut, partitioning);
      if (err != 1)
        mfem_error(
...
#ifndef MFEM_USE_METIS_5
      METIS_PartGraphKway(&n, (idxtype *)I, (idxtype *)J, (idxtype *)NULL,
                          (idxtype *)NULL, &wgtflag, &numflag, &nparts, options,
                          &edgecut, (idxtype *)partitioning);
#else
...
#ifndef MFEM_USE_METIS_5
      METIS_PartGraphVKway(&n, (idxtype *)I, (idxtype *)J, (idxtype *)NULL,
                           (idxtype *)NULL, &wgtflag, &numflag, &nparts,
                           options, &edgecut, (idxtype *)partitioning);
#else
...
#ifdef MFEM_DEBUG
    mfem::out << "Mesh::GeneratePartitioning(...): edgecut = " << edgecut
              << endl;
#endif
  }
...
void FindPartitioningComponents(Table &elem_elem,
                                ...
                                Array<int> &component, Array<int> &num_comp) {
  int i, j, k;
...
void FindTMax(Vector &c, Vector &x, double &tmax, const double factor,
              const int Dim) {
  const double c0 = c(0);
...
void Mesh::CheckDisplacements(const Vector &displacements, double &tmax) {
  ...
  for (int i = 0; i < NumOfElements; i++) {
    ...
    const FiniteElement *fe = GetTransformationFEforElementType(el->GetType());
...
void Mesh::UniformRefinement2D() {
  ...
  static double tri_children[2 * 3 * 4] = {A, A, B, A, A, B, B, B, A, B, B, A,
                                           B, A, C, A, B, B, A, B, B, B, A, C};
...
void Mesh::UniformRefinement3D_base(Array<int> *f2qf_ptr, DSTable *v_to_v_p) {
  ...
  for (int i = 0; i < NumOfElements; i++) {
    ...
    switch (el_type) {
      case Element::TETRAHEDRON: {
        ...
        elements[j + 0] = new Tetrahedron(oedge + e[0], v[1], oedge + e[3],
                                          oedge + e[4], attr);
        elements[j + 1] = new Tetrahedron(oedge + e[1], oedge + e[3], v[2],
                                          oedge + e[5], attr);
      ...
      case Element::WEDGE: {
        ...
        elements[j + 0] =
            new Wedge(oedge + e[1], oedge + e[2], oedge + e[0], oface + qf3,
                      oface + qf4, oface + qf2, attr);
        elements[j + 1] =
            new Wedge(oedge + e[0], v[1], oedge + e[1], oface + qf2,
                      oedge + e[7], oface + qf3, attr);
        elements[j + 2] =
            new Wedge(oedge + e[2], oedge + e[1], v[2], oface + qf4,
                      oface + qf3, oedge + e[8], attr);
        elements[j + 3] = new Wedge(oedge + e[6], oface + qf2, oface + qf4,
                                    v[3], oedge + e[3], oedge + e[5], attr);
        elements[j + 4] =
            new Wedge(oface + qf3, oface + qf4, oface + qf2, oedge + e[4],
      ...
      case Element::HEXAHEDRON: {
        ...
        elements[j + 0] = new Hexahedron(
            oedge + e[0], v[1], oedge + e[1], oface + qf[0], oface + qf[1],
            oedge + e[9], oface + qf[2], oelem + he, attr);
        elements[j + 1] = new Hexahedron(
            oface + qf[0], oedge + e[1], v[2], oedge + e[2], oelem + he,
            oface + qf[2], oedge + e[10], oface + qf[3], attr);
        elements[j + 2] = new Hexahedron(
            oedge + e[3], oface + qf[0], oedge + e[2], v[3], oface + qf[4],
            oelem + he, oface + qf[3], oedge + e[11], attr);
        elements[j + 3] = new Hexahedron(
            oedge + e[8], oface + qf[1], oelem + he, oface + qf[4], v[4],
            oedge + e[4], oface + qf[5], oedge + e[7], attr);
        elements[j + 4] = new Hexahedron(
            oface + qf[1], oedge + e[9], oface + qf[2], oelem + he,
            oedge + e[4], v[5], oedge + e[5], oface + qf[5], attr);
        elements[j + 5] = new Hexahedron(
            oelem + he, oface + qf[2], oedge + e[10], oface + qf[3],
            oface + qf[5], oedge + e[5], v[6], oedge + e[6], attr);
        elements[j + 6] = new Hexahedron(
            oface + qf[4], oelem + he, oface + qf[3], oedge + e[11],
            oedge + e[7], oface + qf[5], oedge + e[6], v[7], attr);
  ...
  for (int i = 0; i < NumOfBdrElements; i++) {
    ...
    if (bdr_el_type == Element::TRIANGLE) {
      ...
      const int qf = (f2qf.Size() == 0) ? be_to_face[i] : f2qf[be_to_face[i]];
  ...
  static double tet_children[3 * 4 * 16] = {
      A, A, A, B, A, A, A, B, A, A, A, B, B, A, A, C, A, A, B, B, A, B, A, B, A,
      B, A, B, B, A, A, C, A, A, B, B, A, A, B, B, A, B, A, B, B, A, A, C,
      // edge coordinates:
      //    0 -> B,A,A  1 -> A,B,A  2 -> A,A,B
      //    3 -> B,B,A  4 -> B,A,B  5 -> A,B,B
      // rt = 0: {0,5,1,2}, {0,5,2,4}, {0,5,4,3}, {0,5,3,1}
      B, A, A, A, B, B, A, B, A, A, A, B, B, A, A, A, B, B, A, A, B, B, A, B, B,
      A, A, A, B, B, B, A, B, B, B, A, B, A, A, A, B, B, B, B, A, A, B, A,
      // rt = 1: {1,0,4,2}, {1,2,4,5}, {1,5,4,3}, {1,3,4,0}
      A, B, A, B, A, A, B, A, B, A, A, B, A, B, A, A, A, B, B, A, B, A, B, B, A,
      B, A, A, B, B, B, A, B, B, B, A, A, B, A, B, B, A, B, A, B, B, A, A,
      // rt = 2: {2,0,1,3}, {2,1,5,3}, {2,5,4,3}, {2,4,0,3}
      A, A, B, B, A, A, A, B, A, B, B, A, A, A, B, A, B, A, A, B, B, B, B, A, A,
      A, B, A, B, B, B, A, B, B, B, A, A, A, B, B, A, B, B, A, A, B, B, A};
  static double pri_children[3 * 6 * 8] = {
      A, A, A, B, A, A, A, B, A, A, A, B, B, A, B, A, B, B, B, B, A, A, B, A,
      B, A, A, B, B, B, A, B, B, B, A, B, B, A, A, C, A, A, B, B, A, B, A, B,
      C, A, B, B, B, B, A, B, A, B, B, A, A, C, A, A, B, B, B, B, B, A, C, B,
      A, A, B, B, A, B, A, B, B, A, A, C, B, A, C, A, B, C, B, B, B, A, B, B,
      B, A, B, B, B, C, A, B, C, B, A, C, B, A, B, C, A, B, B, B, B, B, A, C,
      C, A, C, B, B, C, A, B, B, B, B, B, A, C, B, A, B, C, B, B, C, A, C, C};
  ...
  CoarseFineTr.point_matrices[Geometry::TETRAHEDRON].UseExternalData(
      tet_children, 3, 4, 16);
  CoarseFineTr.point_matrices[Geometry::PRISM].UseExternalData(pri_children, 3,
                                                               6, 8);
  CoarseFineTr.point_matrices[Geometry::CUBE].UseExternalData(hex_children, 3,
                                                              8, 8);
...
void Mesh::LocalRefinement(const Array<int> &marked_el, int type) {
  ...
  if (Dim == 1)  // --------------------------------------------------------
  {
    ...
    CoarseFineTr.point_matrices[Geometry::SEGMENT].UseExternalData(seg_children,
                                                                   1, 2, 3);
  ...
  } else if (Dim == 3)  // ---------------------------------------------------
  {
    ...
    MFEM_VERIFY(
        GetNE() == 0 || ((Tetrahedron *)elements[0])->GetRefinementFlag() != 0,
...
double Mesh::AggregateError(const Array<double> &elem_error, const int *fine,
                            int nfine, int op) {
  double error = 0.0;
  for (int i = 0; i < nfine; i++) {
    ...
    switch (op) {
      case 0:
        error = std::min(error, err_fine);
        break;
      case 1:
        error += err_fine;
        break;
      case 2:
        error = std::max(error, err_fine);
        break;
    }
  }
  return error;
...
bool Mesh::NonconformingDerefinement(Array<double> &elem_error,
  ...
  for (int i = 0; i < dt.Size(); i++) {
    if (nc_limit > 0 && !level_ok[i]) {
      continue;
    }

    double error = AggregateError(elem_error, dt.GetRow(i), dt.RowSize(i), op);
...
void Mesh::EnsureNCMesh(bool triangles_nonconforming) {
  ...
  if (!ncmesh) {
    if ((meshgen & 2) /* quads/hexes */ ||
      ...
      MFEM_VERIFY(GetNumGeometries(Dim) <= 1, "mixed meshes are not supported");
      ncmesh = new NCMesh(this);
      ncmesh->OnMeshUpdated(this);
      GenerateNCFaceInfo();
    }
...
void Mesh::Bisection(int i, const DSTable &v_to_v, int *edge1, int *edge2,
                     int *middle) {
  int *vert;
  ...
  if (t == Element::TETRAHEDRON) {
    ...
    switch (type) {
      case Tetrahedron::TYPE_PU:
        new_type = Tetrahedron::TYPE_PF;
        break;
      case Tetrahedron::TYPE_PF:
        new_type = Tetrahedron::TYPE_A;
        break;
      default:
...
void Mesh::UniformRefinement(int i, const DSTable &v_to_v, int *edge1,
                             int *edge2, int *middle) {
  Array<int> v;
...
void Mesh::Print(adios2stream &out) const {
  ...
  out.io.DefineAttribute("glvis_types", out.glvis_types.data(),
                         out.glvis_types.size());

  size_t varElementsCount = 0;
  // pre-calculate span allocation
  for (auto e = 0; e < NumOfElements; ++e) {
    // geometry type + nvertices + vertices
    varElementsCount += 2 + elements[e]->GetNVertices();
  }

  for (auto e = 0; e < NumOfBdrElements; ++e) {
    // geometry type + nvertices + vertices
    varElementsCount += 2 + boundary[e]->GetNVertices();
  }

  adios2::Variable<double> varVertices = out.io.DefineVariable<double>(
      "vertices", {}, {}, {NumOfVertices, spaceDim}, adios2::ConstantDims);
  adios2::Variable<uint32_t> varElements = out.io.DefineVariable<uint32_t>(
      "elements", {}, {}, {varElementsCount}, adios2::ConstantDims);
  ...
  adios2::Variable<uint32_t>::Span verticesSpan = out.engine.Put(varVertices);
  adios2::Variable<double>::Span elementsSpan = out.engine.Put(varElements);
  ...
  for (auto e = 0; e < NumOfElements; ++e) {
    // geometry type + nvertices + vertices
    elementsSpan[ii] = static_cast<uint32_t>(elements[e]->GetGeometryType());

    const auto NVertices = elements[e]->GetNVertices();
    elementsSpan[ii + 1] = static_cast<uint32_t>(NVertices);

    const int *vertices = elements[e]->GetVertices();

    for (auto v = 0; v < NVertices; ++v) {
      elementsSpan[ii + 2 + v] = static_cast<uint32_t>(vertices[v]);
    }
  ...
  for (auto e = 0; e < NumOfBdrElements; ++e) {
    // geometry type + nvertices + vertices
    elementsSpan[ii] = static_cast<uint32_t>(boundary[e]->GetGeometryType());

    const auto NVertices = boundary[e]->GetNVertices();
    elementsSpan[ii + 1] = static_cast<uint32_t>(NVertices);

    const int *vertices = boundary[e]->GetVertices();
  ...
  for (auto i = 0; i < NumOfVertices; ++i) {
    for (auto j = 0; j < spaceDim; j++) {
      verticesSpan[ii] = vertices[i](j);
      ++ii;
    }
  }

  out.engine.PerformPuts();
}
...
void Mesh::PrintVTK(std::ostream &out, int ref, int field_data) {
  ...
  // write the points
  for (int i = 0; i < GetNE(); i++) {
    RefG = GlobGeometryRefiner.Refine(GetElementBaseGeometry(i), ref, 1);
...
void Mesh::PrintElementsWithPartitioning(int *partitioning, std::ostream &out,
                                         int interior_faces) {
  ...
  else if (meshgen == 2)  // TrueGrid
  {
    ...
    for (i = 0; i < NumOfVertices; i++)
      for (k = 0; k < vcount[i]; k++)
        out << voff[i] + k << " 0.0 " << vertices[i](0) << ' ' << vertices[i](1)
            << ' ' << vertices[i](2) << " 0.0\n";
...
void Mesh::PrintSurfaces(const Table &Aface_face, std::ostream &out) const {
  ...
  for (int iAF = 0; iAF < Aface_face.Size(); ++iAF)
    for (const int *iface = j_AF_f + i_AF_f[iAF];
         iface < j_AF_f + i_AF_f[iAF + 1]; ++iface) {
      out << iAF + 1 << ' ';
...
Mesh *Extrude1D(Mesh *mesh, const int ny, const double sy, const bool closed) {
  ...
  if (!closed) {
    ...
    int nba =
        (mesh->bdr_attributes.Size() > 0 ? mesh->bdr_attributes.Max() : 0);
    for (int i = 0; i < mesh->GetNE(); i++) {
      const Element *elem = mesh->GetElement(i);
      elem->GetVertices(vert);
  ...
  if (nodes) {
    ...
    if (cname == "Linear") {
      ...
      delete mesh2d;
      mfem::err << "Extrude1D : The mesh uses unknown FE collection : " << cname
                << endl;
      mfem_error();
    }
...
Mesh *Extrude2D(Mesh *mesh, const int nz, const double sz) {
  ...
  for (int i = 0; i < mesh->GetNE(); i++) {
    ...
    switch (geom) {
      ...
      case Geometry::SQUARE:
        ...
        break;
      default:
        mfem::err << "Extrude2D : Invalid 2D element type \'" << geom << "\'"
                  << endl;
        mfem_error();
        break;
    }
  ...
  int nba = (mesh->bdr_attributes.Size() > 0 ? mesh->bdr_attributes.Max() : 0);
  for (int i = 0; i < mesh->GetNE(); i++) {
    const Element *elem = mesh->GetElement(i);
    elem->GetVertices(vert);
    const int attr = nba + elem->GetAttribute();
  ...
  if (nodes) {
    ...
    if (cname == "Linear") {
      ...
      delete mesh3d;
      mfem::err << "Extrude3D : The mesh uses unknown FE collection : " << cname
                << endl;
      mfem_error();
    }
