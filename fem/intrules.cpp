...
const IntegrationRule &IntegrationRules::Get(int GeomType, int Order)
{
   ...
   if (Order >= ir_array->Size() || (*ir_array)[Order] == NULL)
   {
      cerr << "\nIntegration rule of order " << Order << " on "
           << Geometry::Name[GeomType] << " is not defined!\n" << endl;
      mfem_error();
   }
...
void IntegrationRules::SegmentIntegrationRules(int refined)
{
   ...
   if (refined)
   {
      ...
      for (i = 1; i < SegmentIntRules.Size(); i += 2)
      {
         n = i/2+1; tmp = new IntegrationRule(n); tmp->GaussianRule();
         SegmentIntRules[i-1] =
            SegmentIntRules[i] = ir = new IntegrationRule(2*n);
         for (j = 0; j < n; j++)
         {
   ...
   for (i = 7; i < SegmentIntRules.Size(); i += 2)
   {
...
void IntegrationRules::TriangleIntegrationRules(int refined)
{
   ...
   */
   ...
   // ir->AddTriPoints3b(7, 0.081414823414553687942,
   //                    0.0475458171336423123969480521942921);
   ...
   ir->AddTriMidPoint(0, 2.62604617004010E-02);
   ir->AddTriPoints3b(1, 9.90363012059100E-03, 5.64007260466500E-03);
   ir->AddTriPoints3b(4, 6.25667297808520E-02, 1.57117591812270E-02);
   ir->AddTriPoints3b(7, 1.70957326397447E-01, 2.35362512520970E-02);
   ir->AddTriPoints3(10, 2.29399572042831E-01, 2.36817932681775E-02);
   ir->AddTriPoints3(13, 1.14424495196330E-01, 1.55837645228970E-02);
   ir->AddTriPoints3(16, 2.48113913634590E-02, 3.98788573253700E-03);
   ir->AddTriPoints6(19, 9.48538283795790E-02, 2.68794997058761E-01,
                     1.84242013643660E-02);
   ir->AddTriPoints6(25, 1.81007732788070E-02, 2.91730066734288E-01,
                     8.70073165191100E-03);
   ir->AddTriPoints6(31, 2.22330766740900E-02, 1.26357385491669E-01,
   ...
   // 61 point - 17 degree (used for 16 as well)
   TriangleIntRules[16] = TriangleIntRules[17] = ir = new IntegrationRule(61);
   ...
   // 73 point - 19 degree (used for 18 as well)
   TriangleIntRules[18] = TriangleIntRules[19] = ir = new IntegrationRule(73);
   ...
   // 126 point - 25 degree (used also for derees from 21 to 24)
   TriangleIntRules[21] = TriangleIntRules[22] = TriangleIntRules[23] =
      TriangleIntRules[24] = TriangleIntRules[25] =
...
void IntegrationRules::TetrahedronIntegrationRules(int refined)
{
   ...
   TetrahedronIntRules.SetSize(22);
   ...
   // 5 points - degree 3 (negative weight)
   ...
   ir->AddTetPoints4(0, 0.21460287125915202929, 6.6537917096945820166E-03);
   ir->AddTetPoints4(4, 0.040673958534611353116, 1.6795351758867738247E-03);
   ...
   // 31 points - degree 7 (negative weight)
   ...
   // 43 points - degree 8 (negative weight)
   TetrahedronIntRules[8] = ir = new IntegrationRule (43);
   ir->AddTetPoints4(0, 5.7819505051979972532E-03, 1.6983410909288737984E-04);
   ir->AddTetPoints4(4, 0.082103588310546723091, 1.9670333131339009876E-03);
   ir->AddTetPoints12(8, 0.036607749553197423679, 0.19048604193463345570,
                      2.1405191411620925965E-03);
   ir->AddTetPoints6(20, 0.050532740018894224426, 4.5796838244672818007E-03);
   ...
   // orders 9 and higher -- Grundmann-Moller rules
   TetrahedronIntRules[9] = new IntegrationRule;
   TetrahedronIntRules[9]->GrundmannMollerTetrahedronRule(4);

   for (int i = 11; i < TetrahedronIntRules.Size(); i += 2)
   {
      TetrahedronIntRules[i-1] = TetrahedronIntRules[i] = new IntegrationRule;
      TetrahedronIntRules[i]->GrundmannMollerTetrahedronRule(i/2);
   }
}
